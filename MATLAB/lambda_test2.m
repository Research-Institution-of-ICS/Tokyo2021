...
for round = 6:10
    ...
    for p = 1:tmax
        ...
        if p<18
            W  = csvread(strcat(cd,'/array/Olympic/array', num2str(p - 1), '.csv'),1, 0)*round/10;
        else
            W  = csvread(strcat(cd,'/array/year array/array', num2str(p - 18), '.csv'), 1, 0)*round/10;
        end
        for j = 1:n
            %% j地区先演变12个小时
            y_1 = [S(j), E(j), I(j), R(j)];
    ...
    save(strcat(cd,'/result/sensitivity/lambda/result',num2str(round),'.mat'), 'I_result','E_result','R_result');
    
    index = max(I_result);
    I_peak = [I_peak;index];
end
save(strcat(cd,'/result/sensitivity/lambda/peak.mat'), 'I_peak');
