...
index_num = [1.2*1/7, 1.6*1/7, 2*1/7, 2.4 * 1/7, 2.8*1/7]; % 希望得到的gamma*r0
count=0;
for round1 = 1:5
    for round2 = 1:5
        count = count+1;
        clearvars -except round1 round2 I_peak index_num count;
        %读取一些数据
        load('data.mat');
        % E表示初始潜伏者人数(认为是疑似病例)
        % S 表示初始状态易感人数
        ...
        gamma =  ones(234,1)*index_num(round1)/R0;
        ...
        save(strcat(cd,'/result/R0_gamma/result',num2str(count),'.mat'), 'I_result','E_result','R_result');
        max_num = max(I_result);
        I_peak= [I_peak;max_num];
    end
end

save(strcat(cd,'/result/R0_gamma/peak.mat'), 'I_peak');
